...
function batch_opf(hour_start_idx, hour_end_idx, zone_grid, timeseries_data, solver, setting, batch_size, file_name::String)
    ...
    for idx in 1 : iterations
        ...
        run_batch_opf(hs_idx, he_idx, zone_grid, timeseries_data, solver, setting, batch_size, file_name, idx)
    end
end

function run_batch_opf(hour_start_idx, hour_end_idx, zone_grid, timeseries_data, solver, setting, batch_size, file_name, idx)
    zone_grid_hourly = deepcopy(zone_grid)
    result = Dict{String, Any}()
    
    for hour_idx in hour_start_idx : hour_end_idx
        #hour_idx =  (idx - 1) * batch_size + hour - (hour_start - 1)
        _EUGO.hourly_grid_data!(zone_grid_hourly, zone_grid, hour_idx, timeseries_data) # write hourly values into the grid data
        result["$hour_idx"] = CBAOPF.solve_cbaopf(zone_grid_hourly, DCPPowerModel, solver; setting = setting) # solve the OPF 
    end
